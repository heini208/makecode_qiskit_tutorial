...
**Tutorial-Version 0.0.7, Stand 12.09.2026**
...
## Das X-Gatter @showdialog

Das **X-Gatter** dreht das Qubit um 180° um die X-Achse. Aus dem Startzustand `|0⟩` wird dadurch `|1⟩`.

![X-Gatter auf der Bloch-Kugel](https://raw.githubusercontent.com/heini208/makecode_qiskit_tutorial/main/images/v007_x_gate.gif)

## X selbst ausprobieren

Ändere jetzt deinen Circuit und teste X selbst.
...
4. Lass **Qubit 0** eingestellt.
5. Drücke **Taste A** mehrmals.

**Erwartung:** Jedes Mal sollte `1` erscheinen, weil X `|0⟩` vollständig nach `|1⟩` dreht.

Klicke nur auf die Glühbirne, wenn du die Lösung als fertige Blockstruktur sehen möchtest.
...
## Das Y-Gatter @showdialog

Das **Y-Gatter** dreht ebenfalls um 180°, aber um die Y-Achse. Auch damit wird aus `|0⟩` bei direkter Messung `|1⟩`.

![Y-Gatter auf der Bloch-Kugel](https://raw.githubusercontent.com/heini208/makecode_qiskit_tutorial/main/images/v007_y_gate.gif)

## Y selbst ausprobieren

Du brauchst keinen neuen Gate-Block. Ändere einfach den vorhandenen.

1. Öffne das Dropdown im Gate-Block.
2. Ändere **X** zu **Y**.
3. Drücke **Taste A** mehrmals.

**Erwartung:** Wieder sollte jedes Mal `1` erscheinen. X und Y führen beide vom Nordpol zum Südpol, aber über unterschiedliche Drehachsen.

Klicke auf die Glühbirne, wenn du die Lösung sehen möchtest.
...
## Das Z-Gatter @showdialog

Das **Z-Gatter** dreht um 180° um die Z-Achse. Es verändert vor allem die Phase des Qubits.

Da das Qubit in `|0⟩` genau auf der Z-Achse startet, verändert eine Z-Drehung seine direkte Wahrscheinlichkeit für `0` und `1` nicht.

## Z selbst ausprobieren

Ändere wieder nur das Dropdown.

1. Öffne das Dropdown im Gate-Block.
2. Ändere **Y** zu **Z**.
3. Drücke **Taste A** mehrmals.

**Erwartung:** Es sollte weiterhin immer `0` erscheinen.
...
## Das Hadamard-Gatter @showdialog

Das **Hadamard-Gatter**, kurz **H**, bringt `|0⟩` in eine gleichgewichtete Superposition.

In der Bloch-Kugel liegt der Zustand danach auf dem Äquator.

![Hadamard-Gatter](https://raw.githubusercontent.com/heini208/makecode_qiskit_tutorial/main/images/v007_hadamard_y90_x180.gif)

## H selbst ausprobieren

Ändere den vorhandenen Gate-Block auf H.

1. Öffne das Dropdown im Gate-Block.
2. Ändere **Z** zu **H**.
3. Drücke **Taste A** mindestens zehnmal.
4. Beobachte, wie oft `0` und wie oft `1` erscheint.

**Erwartung:** `0` und `1` sollten ungefähr gleich häufig erscheinen. Beide Ergebnisse haben eine Wahrscheinlichkeit von 50 %.
...
## Hadamard zweimal @showdialog

Ein H-Gatter kann sich selbst wieder rückgängig machen.

Wenn du **H zweimal direkt hintereinander** anwendest, kehrt das Qubit wieder zu `|0⟩` zurück.

![Zweimal Hadamard](https://raw.githubusercontent.com/heini208/makecode_qiskit_tutorial/main/images/v007_hadamard_then_hadamard.gif)

## H zweimal ausprobieren

Jetzt fügst du einen zweiten H-Block hinzu.

1. Lass das erste **H** stehen.
2. Füge direkt dahinter einen zweiten Grundgatter-Block ein.
3. Öffne dessen Dropdown und wähle ebenfalls **H**.
4. Drücke **Taste A** mehrmals.

**Erwartung:** Jetzt sollte wieder immer `0` erscheinen.

Entferne danach beide H-Gatter, damit du für die Rotationen wieder mit einem leeren Circuit vor der Messung startest.
...
## RX: Drehung um die X-Achse @showdialog

Mit dem Rotationsblock kannst du **RX**, **RY** und **RZ** über ein Dropdown auswählen und zusätzlich einen Winkel einstellen.

Bei **RX 90°** wird das Qubit um 90° um die X-Achse gedreht und landet auf dem Äquator.

![RX mit 90 Grad](https://raw.githubusercontent.com/heini208/makecode_qiskit_tutorial/main/images/v007_rx_pi_2.gif)

## RX selbst ausprobieren

1. Öffne **MicroQiskit → Qiskit Grundlagen → Gatter**.
2. Füge den Rotationsblock vor der Messung ein.
3. Öffne das Dropdown und wähle **RX**.
4. Stelle den Winkel auf **90°**.
5. Drücke **Taste A** mehrfach.

**Erwartung:** Ungefähr 50 % `0` und 50 % `1`.
...
2. Stelle zuerst **45°** ein und drücke Taste A oft.
3. Stelle danach **90°** ein und teste wieder.
4. Stelle zuletzt **135°** ein und teste erneut.

**Erwartung:** Bei 45° sollte `0` deutlich häufiger erscheinen. Bei 90° ungefähr 50/50. Bei 135° sollte `1` deutlich häufiger erscheinen.

Ungefähr gilt: 45° → 85 % `0`, 90° → 50/50, 135° → 85 % `1`.

Klicke auf die Glühbirne, wenn du eine mögliche Einstellung sehen möchtest.
...
## RY: Drehung um die Y-Achse @showdialog

Jetzt drehen wir um die Y-Achse.

![RY mit 90 Grad](https://raw.githubusercontent.com/heini208/makecode_qiskit_tutorial/main/images/v007_ry_pi_2.gif)

## RY selbst ausprobieren
...
**Erwartung:** Bei 90° ungefähr 50/50. Bei 45° häufiger `0`, bei 135° häufiger `1`.
...
## RZ: Drehung um die Z-Achse @showdialog

Zum Schluss drehen wir um die Z-Achse.

![RZ mit 90 Grad](https://raw.githubusercontent.com/heini208/makecode_qiskit_tutorial/main/images/v007_rz_pi_2.gif)

## RZ selbst ausprobieren
...
**Erwartung:** Weiterhin immer `0`. Das Qubit startet auf der Z-Achse, deshalb ändert eine reine RZ-Drehung seine direkte Messwahrscheinlichkeit nicht.
